...
// Function to format a timestamp in nanoseconds since the Unix epoch
string formatTimestamp(int64_t timestamp)
{
    time_t seconds = timestamp / 1'000'000'000; // Convert nanoseconds to seconds
    struct tm timeinfo;
    char buffer[80];
...
void parseMarketDataFeed(const string &filename)
{
    ...
    for (const auto &entry : latest_trade_map)
    {
        ...
        cout << "Listing: " << msg.symbol << ", Last Trade ID: " << msg.trade_id << ", Quantity: " << msg.quantity
             << ", Price: " << msg.price << ", Timestamp: " << formatTimestamp(msg.timestamp) << endl;
    }
